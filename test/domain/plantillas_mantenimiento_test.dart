...
void main() {
  test('un diesel incluye filtro de combustible y no bujias', () {
    final p = plantillasPara(combustible: FuelType.diesel, esAutomatico: false);
  ...
  test('un electrico no incluye aceite de motor', () {
    final p = plantillasPara(
      combustible: FuelType.electrico,
      esAutomatico: false,
    );
  ...
  test(
    'la distribucion se propone sin intervalo, para que lo ponga el dueño',
    () {
      final p = plantillasPara(
        combustible: FuelType.diesel,
        esAutomatico: false,
      );
      final distribucion = p.firstWhere(
        (e) => e.nombre == 'Correa o cadena de distribución',
      );
